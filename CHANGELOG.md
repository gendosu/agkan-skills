...
- タスク管理ワークフローをエージェント非依存に変更し、質問・委譲・計画・スキル参照に実行環境の機能を使用するように改善。委譲できない環境では同じ手順を現在のエージェントで実行
- モデル未指定時は現在のモデルを継承し、明示されたモデルが利用できない場合の停止・スキップ手順を明確化。effort は環境が対応する設定、または作業の入念さを伝える指示として適用
- レビュー手順を特定モデルに依存しない形に整理し、`agkan-run` は実行可能な ready タスクが残っていない場合にスキップしたタスクを報告して終了
...
- **agkan-env-guard**: `PreToolUse` hook (`hooks/env-guard.mjs`) that denies `run_command` calls dumping the whole environment (`env`, `printenv`, `env | grep`, `export -p`, `declare -x`/`-p`, bare `set`, `/proc/*/environ`, `node -e` with whole `process.env`, `python -c` with whole `os.environ`, including inside `$(...)`/backticks/`eval`/`sh -c`) while allowing per-variable access such as `printenv NAME`; non-matching commands are answered with `decision: "ask"` (agy treats a bare `{}` as a deny); covered by `hooks/env-guard.test.mjs` (`node --test`)
...
- **agkan-review**: Use `--json` for `meta get` to fix PR URL fallback parsing
...
- **agkan-subtask-direct**: Fix outdated 'metadata' comment to reflect branch as a first-class column
...
- **agkan-review**: Bundle workflow into `review.sh` script for allowlist registration to reduce permission prompts
...
- **agkan-run**, **agkan-subtask**: Narrow guard condition and enforce PR creation to prevent invalid status transitions

## [0.13.3] - 2026-04-29

### Changed

- **agkan**: Add `task copy` command documentation with usage examples and options

## [0.13.2] - 2026-04-29

### Fixed

- **agkan-review**: Add explicit guard conditions to prevent review status transition when user confirmation is required

## [0.13.1] - 2026-04-27

### Fixed

- **agkan-subtask-direct**, **agkan-subtask**: Add explicit guard conditions to prevent invalid transitions to `done` or `review` status without a commit
